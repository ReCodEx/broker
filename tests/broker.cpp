...
TEST(broker, bind)
{
	...
	EXPECT_CALL(*config, get_client_address()).WillRepeatedly(ReturnRef(address));

	EXPECT_CALL(*config, get_client_port()).WillRepeatedly(Return(1234));

	EXPECT_CALL(*config, get_worker_address()).WillRepeatedly(ReturnRef(address));

	EXPECT_CALL(*config, get_worker_port()).WillRepeatedly(Return(4321));
...
TEST(broker, worker_init)
{
	...
	EXPECT_CALL(*sockets, poll(_, _, _)).InSequence(s1).WillOnce(DoAll(ClearFlags(), SetFlag(message_origin::WORKER)));
	...
	EXPECT_CALL(*router,
		add_worker(AllOf(Pointee(Field(&worker::identity, StrEq("identity1"))),
			Pointee(Field(&worker::headers,
				UnorderedElementsAre(std::pair<std::string, std::string>{"env", "c"},
					std::pair<std::string, std::string>{"hwgroup", "group_1"}))))))
		.InSequence(s2);

	EXPECT_CALL(*sockets, poll(_, _, _)).InSequence(s1).WillOnce(SetArgPointee<2>(true));
...
TEST(broker, queuing)
{
	...
	EXPECT_CALL(*router, find_worker(Eq(headers))).WillRepeatedly(Return(worker));

	EXPECT_CALL(*router, find_worker_by_identity(worker->identity)).WillRepeatedly(Return(worker));
	...
	EXPECT_CALL(*sockets, poll(_, _, _)).InSequence(s1).WillOnce(DoAll(ClearFlags(), SetFlag(message_origin::CLIENT)));
	...
	EXPECT_CALL(*router, deprioritize_worker(worker)).InSequence(s3);

	// Let the worker process it
	EXPECT_CALL(*sockets, send_workers(StrEq(worker->identity), ElementsAre("eval", "job1", "1", "2"))).InSequence(s2);
	...
	EXPECT_CALL(*sockets, recv_workers(_, _, _))
		.InSequence(s2)
		.WillOnce(
			DoAll(SetArgReferee<0>(worker->identity), SetArgReferee<1>(std::vector<std::string>{"done", "job1"})));

	// Give the worker some more work
	EXPECT_CALL(*sockets, send_workers(worker->identity, ElementsAre("eval", "job2", "3", "4"))).InSequence(s2);
