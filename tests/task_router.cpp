...
TEST(task_router, basic_lookup)
{
	...
	auto worker1 =
		task_router::worker_ptr(new worker("id1234", {{"env", "python"}, {"env", "c"}, {"hwgroup", "group_1"}}));

	auto worker2 = task_router::worker_ptr(new worker("id12345", {{"env", "c"}, {"hwgroup", "group_2"}}));

	router.add_worker(worker1);
	router.add_worker(worker2);

	task_router::headers_t headers1 = {{"env", "python"}};

	task_router::headers_t headers2 = {{"hwgroup", "group_2"}};

	task_router::headers_t headers3 = {{"env", "python"}, {"hwgroup", "group_2"}};
...
TEST(task_router, load_balancing)
{
	...
	auto worker1 = task_router::worker_ptr(new worker("id1234", {{"env", "c"}}));

	auto worker2 = task_router::worker_ptr(new worker("id12345", {{"env", "c"}}));
